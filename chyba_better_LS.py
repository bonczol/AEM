...
def steepest_v2(distances, instance):
    ...
    for i, s in enumerate(swap_actions):
        delta = calc_swap_edges_delta_succ((path[s[0] - 1], path[s[0]]), (path[s[1]], path[(s[1] + 1) % len(path)]),
                                           distances)
        if delta > 0:
            add_to_list(LM, ["s", delta, s, (path[s[0] - 1], path[s[0]]), (path[s[1]], path[(s[1] + 1) % len(path)]),
                             "init"])
    ...
    while LM and id < len(LM):
        ...
        if first[0] == "s":
            if first[3][0] in path and first[3][1] in path and first[4][0] in path and first[4][1] in path:
                if np.where(path == first[3][0])[0] == np.where(path == first[3][1])[0] - 1 and \
                        np.where(path == first[4][0])[0] == np.where(path == first[4][1])[0] - 1:
                    # if debug: print("--s------")
            ...
            if first[3] in path and first[4] in outside:
                # if debug: print("--e------")
                path, outside = exchange_vertices_succ(path, outside, first[3], first[4])
                ...
                for i, x in enumerate(path):
                    ...
                    if x_next != first[4] and x != first[4]:
                        out_next = path[(np.where(path == first[4])[0][0]) % len(path)]
                        if out_next != x:
                ...
                for i, x in enumerate(outside):
                    index = (np.where(path == first[4])[0][0]) % len(path)
